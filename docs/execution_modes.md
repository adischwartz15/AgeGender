...
| `"core"` | SimpleCNN (Experiment 0) + Custom ResNet-18 (Experiment D) | Full `MAX_EPOCHS`/`EARLY_STOPPING_PATIENCE` (default 30/8) | The default, "normal" run: the two models needed for the headline SimpleCNN-vs-ResNet efficiency/accuracy comparison, at full training length. Use for a real (non-smoke) single-seed result without the full A-D ablation suite or the residual-connection-specific control. |
...
`[42, 123, 2026]`) additionally re-runs the profile's core experiments
...
| `RUN_FRONTEND_CHECKS` | Whether the frontend build is type-checked/built (never launches the dev server) | Purely a build sanity check; irrelevant to any reported metric. |
...
hardcoded" design. The notebook only selects *which* pre-defined
experiments to run and *how long*, never redefines their architecture
inline. Robustness corruption types/severities are likewise entirely
config-driven from `configs/robustness.yaml`, not the notebook.
