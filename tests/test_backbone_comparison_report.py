"""Tests for the "Plain CNN vs Custom ResNet-18 Backbone Comparison" report section.

Covers both the pure table-building function and the full report
assembly's honest "not yet run" fallback when one or both experiments'
...
def test_backbone_comparison_section_renders_table_when_both_present(tmp_path):
...
def test_generate_markdown_report_includes_backbone_comparison_heading(tmp_path):
    ...
    assert "## Plain CNN vs Custom ResNet-18 Backbone Comparison" in report
